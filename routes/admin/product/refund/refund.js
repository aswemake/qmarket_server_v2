...
router.get('/', async (req, res) => {
    if (!req.signedCookies.user) {
        ...
        try {
            ...
            console.log(data);
            res.render('product_refund', { data });
        } catch (err) {
            console.log(err);
    ...
    if (!req.signedCookies.user) {
        ...
        if (!refund_id || !order_id) {
            ...
            try {
                ...
                // 환불 테이블
                let get_refund_query = 'SELECT * FROM refunds WHERE refund_id = ? AND order_id = ? AND is_refund = 1';
                ...
                if (!order_info[0] || !get_refund[0]) {
                    ...
                    // 환불테이블 업데이트 쿼리
                    let refund_coupon_query = 'UPDATE refunds SET coupon_price = ?, coupon_idx = ?, is_refund = ? WHERE refund_id = ?'
                    let refund_qmoney_query = 'UPDATE refunds SET qmoney = ?, is_refund = ? WHERE refund_id = ?'
                    let refund_price_query = 'UPDATE refunds SET price = ?, qmoney = ?, delivery_price = ?, is_refund = ? WHERE refund_id = ?'

                    // 환불여부 검사
                    let check_refund_query = 'SELECT price, qmoney, coupon_idx FROM refunds WHERE order_id = ? AND is_refund = 2';
                        ...
                        if (order_info[0].coupon_idx) {
                            ...
                            if (refund_price <= order_info[0].coupon_price) {
                                if (coupon_check == true) {
                                    // 환불 테이블 업데이트
                                    let update_refund = await connection.query(refund_coupon_query, [refund_price, order_info[0].coupon_idx, 2, refund_id]);
                                } else {
                                    ...
                                    // 환불 테이블 업데이트
                                    let update_refund = await connection.query(refund_coupon_query, [refund_price, null, 2, refund_id]);
                                }
                                ...
                                if (coupon_check == false) {
                                    ...
                                    console.log(update_qmoney);
                                    // 환불 테이블 업데이트
                                    let update_refund = await connection.query(refund_coupon_query, [order_info[0].coupon_price, null, 1, refund_id]);
                                    console.log(update_refund);
                                } else {
                                    let update_refund = await connection.query(refund_coupon_query, [order_info[0].coupon_price, order_info[0].coupon_idx, 1, refund_id]);
                                }
                    ...
                    if (refund_price <= qmoney) {
                        ...
                        // 환불 테이블 업데이트
                        let update_refund = await connection.query(refund_qmoney_query, [refund_price, 2, refund_id]);
                        ...
                        if (!merchant_uid) {
                            ...
                            if (get_refund[0].is_all == 0) {
                                let update_refund = await connection.query(refund_price_query, [refund_price, qmoney, 0, 2, refund_id]);
                                console.log(`update_refund:${update_refund[0]}`);
                            } else if (get_refund[0].is_all == 1) {
                                let update_refund = await connection.query(refund_price_query, [(refund_price - order_info[0].delivery_price), qmoney, order_info[0].delivery_price, 2, refund_id]);
                                console.log(`update_refund:${update_refund[0]}`);
                            }

                            // 배송 테이블 업데이트
                            let update_order_query = "UPDATE orders SET is_delivery = ? WHERE order_id = ?"
                            let update_order = await connection.query(update_order_query, [6, order_id]);
    ...
    if (!req.signedCookies.user) {
        ...
        if (!refund_id || !order_id || !content) {
            ...
            try {
                ...
                let query = 'UPDATE refunds SET reject_reason = ?, is_refund = 3 WHERE refund_id = ? AND order_id = ?';
                let update_refund = await connection.query(query, [content, refund_id, order_id]);
                if (update_refund.affectedRows === 1) {
