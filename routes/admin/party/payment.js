...
router.get('/', async (req, res) => {
    if (!req.signedCookies.user) {
        ...
        try {
            ...
            let query = 'SELECT party_id, user_idx, count, payment, created_at FROM orders_parties';
            let query2 = 'SELECT name FROM users WHERE user_idx = ?';
            let payment_info = await connection.query(query);
            for (let i = 0; i < payment_info.length; i++) {
                let payment_data = new Object();
                let get_party = await Party.find({_id: payment_info[i].party_id}).select({no: 1, name: 1});
